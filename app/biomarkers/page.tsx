...
export default function BiomarkersDatabase() {
  ...
  const filteredBiomarkers = biomarkers.filter(biomarker => {
    const matchesSearch = biomarker.name.toLowerCase().includes(searchTerm.toLowerCase()) ||
                         biomarker.use?.toLowerCase().includes(searchTerm.toLowerCase());
    ...
    <>
      ...
      <nav className="relative z-10 flex items-center justify-between px-16 py-10 animate-fadeIn animation-delay-200 opacity-0" style={{ animationFillMode: 'forwards' }}>
        <div className="flex items-center gap-16">
          ...
          <div className="hidden lg:flex items-center gap-10">
            ...
            <Link href="/biomarkers" className="text-orange-500 hover:text-orange-400 hover:underline transition-all font-mono text-sm">Biomarkers</Link>
      ...
      <div className="relative z-10 container mx-auto px-16 py-12 pb-20">
        ...
        <p className="text-xl text-gray-300 font-mono max-w-2xl animate-fadeInUp opacity-0" style={{ animationDelay: '800ms', animationFillMode: 'forwards' }}>
          Explore clinically measurable biomarkers used for diagnostics and health monitoring. 
          Understanding your biomarkers is key to optimizing your health.
        </p>
      </div>
      ...
      <section className="relative py-24">
        ...
        <div className="bg-white/95 backdrop-blur rounded-2xl shadow-xl p-8 mb-8 animate-fadeInUp opacity-0" style={{ animationDelay: '200ms', animationFillMode: 'forwards' }}>
          <div className="flex flex-col md:flex-row gap-4 mb-6">
            ...
            <select
              ...
              className="px-6 py-3 border border-gray-300 rounded-md font-mono text-sm focus:outline-none focus:border-orange-500 focus:ring-2 focus:ring-orange-200 transition-all"
            >
          ...
          <div className="flex items-center justify-between">
            <p className="text-gray-600 font-mono text-sm">
              Showing <span className="font-semibold text-gray-900">{filteredBiomarkers.length}</span> biomarkers
        ...
        {loading && (
          <div className="text-center py-12">
            ...
            <p className="mt-4 text-gray-600 font-mono">Loading biomarkers data...</p>
          </div>
        )}
        ...
        {!loading && !error && viewMode === 'card' && (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredBiomarkers.map((biomarker, index) => (
              <Link
                key={biomarker.id}
                href={`/biomarkers/${encodeURIComponent(biomarker.id)}`}
                className="block"
              >
                <div
                  className="bg-white/95 backdrop-blur rounded-xl shadow-md hover:shadow-xl transition-all p-6 border border-gray-200 hover:border-blue-200 animate-fadeInUp opacity-0 cursor-pointer h-full flex flex-col"
                  style={{ animationDelay: `${(index % 6) * 100 + 400}ms`, animationFillMode: 'forwards' }}
                >
                  <h3 className="text-xl font-normal mb-3 font-headline text-gray-900">
                    {biomarker.name}
                  </h3>
                  {biomarker.category && (
                    <span className="inline-block px-3 py-1 bg-blue-100 text-blue-700 rounded-full text-xs font-mono mb-3 uppercase">
                      {biomarker.category}
                    </span>
                  )}
                  <p className="text-gray-600 font-mono text-sm line-clamp-3 flex-grow">
                    {biomarker.use || 'Clinical biomarker for health monitoring'}
                  </p>
                  <p className="mt-4 text-blue-600 hover:text-blue-700 font-mono text-sm hover:underline transition-all">
                    View Details →
                  </p>
                </div>
              </Link>
              ...
              <table className="w-full">
                <thead className="bg-gray-50 border-b">
                  <tr>
                    ...
                    <th className="px-6 py-4 text-left text-sm font-mono text-gray-700 uppercase tracking-wider">Type</th>
                    <th className="px-6 py-4 text-left text-sm font-mono text-gray-700 uppercase tracking-wider">Clinical Use</th>
                ...
                <tbody className="divide-y divide-gray-200">
                  {filteredBiomarkers.map((biomarker, index) => (
                    <tr key={biomarker.id} className="hover:bg-gray-50 transition-colors">
                      ...
                      <td className="px-6 py-4">
                        {biomarker.category && (
                          <span className="inline-block px-3 py-1 bg-blue-100 text-blue-700 rounded-full text-xs font-mono uppercase">
                            {biomarker.category}
                          </span>
                        )}
                      </td>
                      ...
                      <td className="px-6 py-4">
                        <Link href={`/biomarkers/${encodeURIComponent(biomarker.id)}`} className="text-blue-600 hover:text-blue-700 font-mono text-sm hover:underline">
                          View Details →
                        </Link>
                      </td>
                    </tr>
